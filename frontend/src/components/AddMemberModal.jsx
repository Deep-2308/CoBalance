  ...
  const handleImportContacts = async () => {
    ...
    try {
      ...
      if ('contacts' in navigator && 'ContactsManager' in window) {
        ...
        const selectedContactsList = await navigator.contacts.select(props, opts);
        
        const formattedContacts = selectedContactsList.map((contact, index) => ({
          id: `contact-${index}`,
        ...
        alert('Contact picker not supported on this browser. Please use manual entry.');
        setMethod('manual');
      }
    } catch (err) {
                  ...
                  <div className="max-h-96 overflow-y-auto mb-4">
                    {contacts.map((contact) =>
                      contact.phones.map((phone, idx) => {
                        const key = `${contact.id}-${phone}`;
                        const isSelected = selectedContacts.some(c => c.key === key);
